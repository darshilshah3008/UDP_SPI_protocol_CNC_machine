...
uint8_t etherGetRevId() {
    etherSetBank(EREVID);
    uint8_t rev = etherReadReg(EREVID);
    return rev;
}
...
void etherCalcIpChecksum(ipFrame* ip)
{
    ...
    sum = 0;
    ip->headerChecksum = 0;
    etherSumWords(ip, (ip->revSize & 0xF) * 4);
...
//    tmp16 = udp->sourcePort;
    udp->sourcePort = udp->destPort;
    udp->destPort = htons(clientPort);
...
uint16_t FindDestinationPort(uint8_t packet[])
{
    uint16_t destPort;
    etherFrame* ether = (etherFrame*)packet;
    ipFrame* ip = (ipFrame*)&ether->data;
    udpFrame* udp = (udpFrame*)((uint8_t*)ip + ((ip->revSize & 0xF) * 4));
    destPort=htons(udp->destPort);
    return destPort;
}
